...
def main(_):
    ...
    document_dir_path = os.path.join(FLAGS.dataset_dir, '*', '*', '*', 'PROC')
    data_file_pattern = os.path.join(document_dir_path, '*-DataFix-__RAW__-*.xml')
    xml_file_list = glob(data_file_pattern)

    pascalvoc_to_tfrecords.run(xml_file_list, FLAGS.output_dir, FLAGS.dataset_name, FLAGS.output_name)
