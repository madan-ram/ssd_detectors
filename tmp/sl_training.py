...
class SegLinkLoss(object):
    ...
    def compute(self, y_true, y_pred):
        ...
        seg_class_pred = tf.argmax(input=seg_conf_pred, axis=1)
        ...
        num_seg = tf.cast(tf.shape(input=seg_conf_true)[0], tf.float32)
        num_pos_seg = tf.reduce_sum(input_tensor=pos_seg_mask_float)
        num_neg_seg = num_seg - num_pos_seg
        
        pos_seg_conf_loss = tf.reduce_sum(input_tensor=seg_conf_loss * pos_seg_mask_float)
        ...
        neg_seg_conf_loss = tf.boolean_mask(tensor=seg_conf_loss, mask=neg_seg_mask)
        
        vals, idxs = tf.nn.top_k(neg_seg_conf_loss, k=tf.cast(num_neg_seg, tf.int32))
        neg_seg_conf_loss = tf.reduce_sum(input_tensor=vals)
        ...
        pos_seg_loc_loss = tf.reduce_sum(input_tensor=seg_loc_loss * pos_seg_mask_float)
        ...
        link_class_pred = tf.argmax(input=link_conf_pred, axis=1)
        ...
        num_link = tf.cast(tf.shape(input=link_conf_true)[0], tf.float32)
        num_pos_link = tf.reduce_sum(input_tensor=pos_link_mask_float)
        num_neg_link = num_link - num_pos_link
        
        pos_link_conf_loss = tf.reduce_sum(input_tensor=link_conf_loss * pos_link_mask_float)
        ...
        neg_link_conf_loss = tf.boolean_mask(tensor=link_conf_loss, mask=neg_link_mask)
        
        vals, idxs = tf.nn.top_k(neg_link_conf_loss, k=tf.cast(num_neg_link, tf.int32))
        neg_link_conf_loss = tf.reduce_sum(input_tensor=vals)
        ...
        seg_conf = tf.reduce_max(input_tensor=seg_conf_pred, axis=1)
        seg_class_true = tf.argmax(input=seg_conf_true, axis=1)
        seg_class_pred = tf.argmax(input=seg_conf_pred, axis=1)
        ...
        link_conf = tf.reduce_max(input_tensor=link_conf_pred, axis=1)
        link_class_true = tf.argmax(input=link_conf_true, axis=1)
        link_class_pred = tf.argmax(input=link_conf_pred, axis=1)
...
class SegLinkFocalLoss(object):
    ...
    def compute(self, y_true, y_pred):
        ...
        seg_conf_loss = tf.reduce_sum(input_tensor=seg_conf_loss)
        ...
        inter_link_conf_loss = tf.reduce_sum(input_tensor=inter_link_conf_loss)
        num_inter_links = tf.cast(tf.shape(input=inter_link_conf_true)[0], tf.float32)
        ...
        cross_link_conf_loss = tf.reduce_sum(input_tensor=cross_link_conf_loss)
        num_cross_links = tf.cast(tf.shape(input=cross_link_conf_true)[0], tf.float32)
        ...
        inter_link_conf = tf.reduce_max(input_tensor=inter_link_conf_pred, axis=1)
        inter_link_class_true = tf.argmax(input=inter_link_conf_true, axis=1)
        inter_link_class_pred = tf.argmax(input=inter_link_conf_pred, axis=1)
        ...
        cross_link_conf = tf.reduce_max(input_tensor=cross_link_conf_pred, axis=1)
        cross_link_class_true = tf.argmax(input=cross_link_conf_true, axis=1)
        cross_link_class_pred = tf.argmax(input=cross_link_conf_pred, axis=1)
